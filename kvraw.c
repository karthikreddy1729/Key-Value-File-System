...
struct backup *kvraw_restore(struct kvraw *kvraw){
    struct backup * bkup = logfs_restore(kvraw->logfs);
    if(bkup->valid){
        memcpy(&kvraw->size, (void*)bkup->limit, sizeof(uint64_t));
...
void
kvraw_backup(struct kvraw *kvraw){

    logfs_backup(kvraw->logfs);
	struct backup * bkup = get_backup();

    memcpy((void*)bkup->limit, &kvraw->size, sizeof(uint64_t));
	bkup->limit += sizeof(uint64_t);
    return;
}
...
void
kvraw_backup_flush(struct kvraw *kvraw){
    logfs_backup_flush(kvraw->logfs);
}
