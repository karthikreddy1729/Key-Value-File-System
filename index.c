...
struct backup *index_restore(struct index *index, struct backup * bkup){
    if(bkup->valid){
		uint64_t capacity;

		memcpy(&capacity, (void*)bkup->limit, sizeof(uint64_t));
		bkup->limit += sizeof(uint64_t);


		destroy(index);
		create(index, capacity);

		memcpy(&index->size, (void*)bkup->limit, sizeof(uint64_t));
		bkup->limit += sizeof(uint64_t);

		memcpy(index->maps, (void*)bkup->limit, capacity * sizeof (index->maps[0]));
		bkup->limit += capacity * sizeof (index->maps[0]);
...
void
index_backup(struct index *index){

	struct backup * bkup = get_backup();


	memcpy((void*)bkup->limit, &index->capacity, sizeof(uint64_t));
	bkup->limit += sizeof(uint64_t);

    memcpy((void*)bkup->limit, &index->size, sizeof(uint64_t));
	bkup->limit += sizeof(uint64_t);


	memcpy((void*)bkup->limit, index->maps, index->capacity * sizeof (index->maps[0]));
	bkup->limit += index->capacity * sizeof (index->maps[0]);
