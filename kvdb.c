...
void kvdb_restore(struct kvdb *kvdb){
    struct backup * bkup = kvraw_restore(kvdb->kvraw);
	index_restore(kvdb->index, bkup);

    if(bkup->valid){
        memcpy(&kvdb->size, (void*)bkup->limit, sizeof(uint64_t));
		bkup->limit += sizeof(uint64_t);

		memcpy(&kvdb->waste, (void*)bkup->limit, sizeof(uint64_t));
		bkup->limit += sizeof(uint64_t);
    }
	clean(bkup);
    return;
...
void
kvdb_backup(struct kvdb *kvdb){
    kvraw_backup(kvdb->kvraw);
	index_backup(kvdb->index);


	struct backup * bkup = get_backup();


    memcpy((void*)bkup->limit, &kvdb->size, sizeof(uint64_t));
	bkup->limit += sizeof(uint64_t);

	memcpy((void*)bkup->limit, &kvdb->waste, sizeof(uint64_t));
	bkup->limit += sizeof(uint64_t);
    return;
}

void
kvdb_backup_flush(struct kvdb *kvdb){
    kvraw_backup_flush(kvdb->kvraw);
}
...
struct kvdb *
kvdb_open(const char *pathname, int restore)
{
	struct kvdb *kvdb;
...
void
kvdb_close(struct kvdb *kvdb, int backup)
{
	if(backup){
	kvdb_backup(kvdb);
	kvdb_backup_flush(kvdb);
	}
	if (kvdb) {
